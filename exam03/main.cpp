...
void acc(void) {
   ...
   while (true) {
    ...
    horizontal_velocity = 9.8 * sqrt((r[0]-t[0])*(r[0]-t[0]) + (r[1]-t[1])*(r[1]-t[1])) * (0.1); //horizontal_velocity = (9.8)*(a)*t // t = 0.1
...
void getAcc(Arguments *in, Reply *out) {
   pc.printf("FXOS8700Q horizontal velocity = %2.4f(%x%x)", horizontal_velocity);
}
